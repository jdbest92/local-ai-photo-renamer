...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.recursive_check.setToolTip(
            "Inclure les photos des sous-dossiers (les fichiers renommes restent "
        ...
        self.scan_btn.setToolTip(
            "Le filtre s'appuie sur les visages deja analyses (cache) et sur les noms "
            "presents dans les fichiers. Ce bouton analyse les photos restantes en arriere-plan.")
        ...
        tabs.addTab(self.enroll_tab, "Enrolement")
        ...
        self.log_bus.message.emit("Application demarree. Ouvrez un dossier de photos pour commencer.")
    ...
    def open_folder(self, folder):
        ...
        self.statusBar().showMessage(f"{n} photo(s) chargee(s).", 5000)
    ...
    def _on_recursive_toggled(self, checked):
        ...
        if self.current_folder:
            ...
            self.log_bus.message.emit(
                ...
                "grille rechargee, relancez l'analyse si besoin.")
    ...
    def apply_person_filter(self, person):
        ...
        if not_cached:
            msg += (f" {not_cached} photo(s) pas encore analysee(s) : "
    ...
    def scan_all_for_filter(self):
        ...
        self._scan_worker.progress.connect(
            lambda i, n: self.statusBar().showMessage(f"Analyse des visages : {i}/{n}"))
        self._scan_worker.finished_ok.connect(self._on_scan_done)
        self.scan_btn.setText("Arreter l'analyse")
        self.log_bus.message.emit("--- Analyse du dossier pour le filtre par personne ---")
        self.statusBar().showMessage(
            "Analyse demarree (chargement du modele au premier passage, "
    ...
    def _on_scan_done(self):
        self.scan_btn.setText("Analyser tout le dossier (pour le filtre)")
        self.statusBar().showMessage(
            "Analyse des visages terminee : le filtre par personne est a jour. "
            "(Pour renommer, utilisez 'Analyser le dossier (apercu)' dans l'onglet Renommage.)",
            15000)
        self.log_bus.message.emit(
            "Analyse des visages terminee. Cette analyse alimente le filtre par personne ; "
